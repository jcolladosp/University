...
/* Devuelve el n�mero de iteraciones realizadas */
                   ...
                   int yo, int np)
{
  ...
  if(yo == 0) {
    ...
    for ( filas_hechas = 0 ; filas_hechas < h ; filas_hechas++ ) {
	  ...
	  // Receive a computed row in a nonblocking way
	  MPI_Irecv(B, w, MPI_BYTE, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &request);
	  MPI_Test(&request, &flag, &status);

	  // While the thread is not ready, compute rows
	  while (!flag && siguiente_fila < h) {
	  	// Begin process row=siguiente_fila
		z0.b = y1 + iy*siguiente_fila;
        for ( j = 0 ; j < w ; j++ ) {
          z0.a = x1 + ix*j;
          ni = newton(z0, tol, maxiter);
          if ( ni > max ) max = ni;
          A(siguiente_fila, j) = ni;
        }
		// End process row
		// Update counters and check irecv
		siguiente_fila++;
		filas_hechas++;
		MPI_Test(&request, &flag, &status);
	  }

	  // If the receive process hasn't ended, wait for it
	  if (!flag) MPI_Wait(&request, &status);
	  // Continue as master
      /* Sacamos el identificador del proceso y el n�mero de fila */
      proc = status.MPI_SOURCE;
      /*
        Para ahorrarnos un mensaje extra, el trabajador pone el n�mero de fila
        en el TAG del mensaje.
        Tambi�n podr�amos habernos apuntado en el maestro qu� fila procesa cada
        trabajador.
      */
    ...
    /* C�DIGO DE LOS TRABAJADORES */

    /* Recibe n�mero de fila con la que trabajar, fuera de rango para acabar */
    ...
    while ( num_fila < h ) {
      ...
      /* Env�a fila reci�n calculada */
      MPI_Send(B, w, MPI_BYTE, 0, num_fila, MPI_COMM_WORLD);
      /* Recibe n�mero de la siguiente fila a procesar */
      MPI_Recv(&num_fila, 1, MPI_INT, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }

  }

  /* Queda calcular el m�ximo de todos los m�ximos */
  ni = max;
  ...
  while (argc > 1) {
    if (argv[1][0] == '-') {
      ...
      case 'i':
        ...
        /* �Ojo! maxiter no debe valer m�s de 255 */
