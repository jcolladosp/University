...
public class TablaHash<C, V> implements Map<C, V> {
    ...
    public TablaHash(int inicial) {
       ...
       public ListaConPI<C> clavesEnLongitud(int n) { 
           ListaConPI<C> sol = new LEGListaConPI<>();
          
           for (int i = 0; i < elArray.length; i++) { 
                ListaConPI<EntradaHash<C, V>> xd = elArray[i];
               
                if(xd.talla() == n){
               
               for(xd.inicio();!xd.esFin();xd.siguiente()){
                sol.insertar(xd.recuperar().clave);
                }
    ...
    public ListaConPI<C> claves() {
        ...
        for(int i=0; i < elArray.length;i++){
              ...
              ListaConPI<EntradaHash<C, V>> elemento = elArray[i];
    ...
    public final double desviacionTipica() {
        ...
        for(int i = 0; i < elArray.length;i++){
        sol = sol + Math.pow(elArray[i].talla() - factorCarga,2);
        }
        
        sol = Math.sqrt(sol / elArray.length);
        return sol;
