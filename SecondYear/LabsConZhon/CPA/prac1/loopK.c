...
#define NUM_PASOS 5
#define DIST_RADIO 8
#define IMAGEN_ENTRADA "lenatocha.ppm"
#define IMAGEN_SALIDA "lennatocha2.ppm"
  ...
  for (p = 0; p < pasos; p++) {
    for (i = 0; i < n; i++) {
      for (j = 0; j < m; j++) {
        ...
        rb = 0;
        tot = 0;
#pragma omp parallel for private(l) reduction(+:tot,rr,rg,rb)
  ...
  for (p = 0; p < pasos; p++) {
    for (i = 0; i < n; i++) {
      for (j = 0; j < m; j++) {
        ...
        rb = 0;
        tot = 0;
#pragma omp parallel for private(v,resultado,l) reduction(+:tot)
        for (k = max(0, i - radio); k <= min(n - 1, i + radio); k++) {
          for (l = max(0, j - radio); l <= min(m - 1, j + radio); l++) {
            v = ppdBloque[k - i + radio][l - j + radio];
            rr += ppsImagenOrg[k][l].r * v;
